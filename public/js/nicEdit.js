...
var nicEditorConfig = bkClass.extend({
    ...
    iconsPath: asset_path+'images/nicEditIcons-latest.gif',
...
var nicLinkOptions = {
    buttons: {
        'link': { name: 'Thêm Liên Kết', type: 'nicLinkButton', tags: ['A'] },
        'unlink': { name: 'Xóa Liên Kết', command: 'unlink', noActive: true }
    }
};
...
iconFiles: {
    'youTube': asset_path+'images/youtube-icon.png'
}
};
...
var nicColorOptions = {
    buttons: {
        ...
        'bgcolor': { name: __('Đổi Màu Nền'), type: 'nicEditorBgColorButton', noClose: true }
    }
};
...
var nicImageOptions = {
    buttons: {
        'image': { name: 'Thêm Hình Ảnh', type: 'nicImageButton', tags: ['IMG'] }
    }
...
var nicUploadButton = nicEditorAdvancedButton.extend({ 
    addPane: function () { 
       ...
       // this.im = this.ne.selectedInstance.selElm().parentTag("IMG");
        ...
        new bkElement("div").setStyle({ fontSize: "14px", fontWeight: "bold", paddingBottom: "5px" }).setContent("Thêm Hình Ảnh").appendTo(A); 
       
        this.fileInput = new bkElement("input").setAttributes({ type: "button","value":"Chọn Hình Ảnh" }).appendTo(A); 
        // this.progress = new bkElement("progress").setStyle({ width: "100%", display: "none" }).setAttributes("max", 100).appendTo(A); 
        var parent=this;

        this.fileInput.onclick = function(){
            parent.removePane();
            $(".nicupload.showupload").click();

            btnUpload="#"+parent.ne.nicInstances[0].e.id;
        }
        ...
        //             this.onError("Only image files can be uploaded"); return 
        //         } 
